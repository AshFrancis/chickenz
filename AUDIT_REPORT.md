...
2. **H5** — DER signature bounds validation
3. **H6** — Token expiry check at use time
4. **H8** — Odd consolation player dropping
5. **H12** — Spectator cap
6. **H15** — Clear tournament animation timeouts
7. **H18** — Re-enable lobby buttons on join failure
8. **M1** — Unicode normalization in profanity filter
9. **M2** — Rate limiter max entries

### Next Sprint

15. **H10** — Server-side reconnection grace period
2. **H17** — Wallet error feedback
3. **M3** — Input tick window validation
4. **M14** — Use full UUIDs for IDs
5. **M15** — Clean up GameScene event listeners
6. **M20** — Disable settle button during settlement
