...
#   SERVER_URL=http://localhost:3000 ./scripts/worker.sh   # override (single server)
#
...
# Default: poll all region servers. Override with SERVER_URL for single-server mode.
  ...
  for SERVER_URL in $SERVERS; do
    ...
    if (cd "$(dirname "$ARTIFACTS_FILE")" && "$PROVER_BINARY" "$TMPFILE" 2>"$STDERR_FILE"); then
      ...
      if [[ -f "$ARTIFACTS_PATH" ]]; then
        ...
        echo "  Submitted to $SERVER_URL: $RESULT"
        rm -f "$ARTIFACTS_PATH"
      else
